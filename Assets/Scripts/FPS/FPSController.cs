...
public class FPSController : MonoBehaviour
    ...
    [SerializeField]
    private bool m_LimitDiagonalSpeed = true;
    ...
    private void Update()
    {
        ...
        if (m_ToggleRun && m_Grounded && m_Input.ToggleRun)
        {
            m_Speed = (m_Speed == m_WalkSpeed ? m_RunSpeed : m_WalkSpeed);
        }

        //If both horizontal and vertical are used simultaneously, limit speed (if allowed), so the total doesn't exceed normal move speed
        float inputModifyFactor = (m_Input.Movement.x != 0.0f && m_Input.Movement.y != 0.0f 
                                    && m_LimitDiagonalSpeed) ? .7071f : 1.0f;
        ...
        if (m_Grounded)
        {
            ...
            if (!m_ToggleRun)
            {
                m_Speed = m_Input.ToggleRun ? m_RunSpeed : m_WalkSpeed;
            }
            ...
            {
                m_MoveDirection = new Vector3(m_Input.Movement.x * inputModifyFactor, -m_AntiBumpFactor,
                                              m_Input.Movement.y * inputModifyFactor);
        ...
        else
        {
            ...
            {
                m_MoveDirection.x = m_Input.Movement.x * m_Speed * inputModifyFactor;
                m_MoveDirection.z = m_Input.Movement.y * m_Speed * inputModifyFactor;
