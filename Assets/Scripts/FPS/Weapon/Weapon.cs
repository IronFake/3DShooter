...
public class Weapon : MonoBehaviour
{
    ...
    public void PickedUp(WeaponManager wm)
    {
        weaponManager = wm;
    }
    ...
    private Ray GetRay()
    {
        if (weaponManager.OwnerIsPlayer)
        {
            ...
            float spreadRatio = advancedSettings.spreadAngle / weaponManager.mainCamera.fieldOfView;
            Vector2 spread = spreadRatio * Random.insideUnitCircle;

            return weaponManager.mainCamera.ViewportPointToRay(Vector3.one * 0.5f + (Vector3)spread);
        }
        else
        {
    ...
    public Vector3 GetCorrectedMuzzlePlace()
    {
        ...
        position = weaponManager.weaponCamera.WorldToScreenPoint(position);
        position = weaponManager.mainCamera.ScreenToWorldPoint(position);
